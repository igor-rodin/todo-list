...
def add(): # Добавление данных
    ...
    add2log(f'Title = {title}; Note = {note}; Status = {status}','<')
    return {
            'title' : title,
            'note' : note,
            'status' : status,
...
def show_records(local_list): # Отображение всей базы на экране красиво
    ...
    for i in range(len(local_list)):
        print(f'{i:7}|{local_list[i]["title"]:59}|{local_list[i]["status"]:12}|{local_list[i]["datetime"]:11}')
    print(f'{"-"*7}+{"-"*59}+{"-"*12}+{"-"*19}')
...
def edit_record(local_list): # Интерфейс редактирования записи
    ...
    if len(inp) > 0: status = inp
    else: status = local_list["status"]
    datetime = time.strftime("%Y.%m.%d %H:%M:%S",time.gmtime(time.time()))
    add2log(f'Редактирование записи title {local_list["title"]} => {title}; {local_list["note"]} => {note};{local_list["status"]} => {status}','<')
    return {
            'title' : title,
            'note' : note,
            'status' : status,
